...
<?= view('components/_breadcrumb', ['items' => [['label' => 'Dashboard', 'url' => base_url('guru')], ['label' => 'Profil']]]) ?>
...
<div class="max-w-2xl mx-auto">
    <div class="glass-card rounded-3xl overflow-hidden shadow-sm shadow-primary/5">
        ...
        <form method="post" action="<?= base_url('guru/profil/update') ?>" data-offline="true" class="p-8 space-y-6">
            <?= csrf_field() ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="fieldNip" class="text-xs text-outline uppercase tracking-wider font-semibold block mb-1">NIP</label>
                    <input type="text" id="fieldNip" value="<?= esc($user['nip'] ?? '—') ?>" readonly disabled
                ...
                <div>
                    <label for="fieldRole" class="text-xs text-outline uppercase tracking-wider font-semibold block mb-1">Role</label>
                    <input type="text" id="fieldRole" value="<?= esc($user['role_kode'] ?? '—') ?>" readonly disabled
            ...
            <div class="border-t border-outline-variant/20 pt-6">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <span class="px-3 py-1.5 bg-surface-container-low rounded-xl text-sm text-on-surface-variant">
                        <?= esc($m['mapel_nama']) ?>
                        <?php if ($m['rombel_nama']): ?><span class="text-outline text-xs ml-1">(<?= esc($m['rombel_nama']) ?>)</span><?php endif; ?>
                        <?php if ($m['tahun_ajar_nama']): ?><span class="text-outline/50 text-[10px] ml-1"><?= esc($m['tahun_ajar_nama']) ?></span><?php endif; ?>
                    </span>
                    <?php endforeach; ?>
                </div>
            </div>
