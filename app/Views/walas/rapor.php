...
<?= $this->section('title') ?>Rapor Kelas<?= $this->endSection() ?>
<?= $this->section('sidebar') ?><?= $this->include('layouts/sidebar_walas') ?><?= $this->endSection() ?>

<?= $this->section('content') ?>
<?= view('components/_breadcrumb', ['items' => [['label' => 'Dashboard', 'url' => base_url('walas')], ['label' => 'Rapor Kelas']]]) ?>
<div class="flex items-center justify-between mb-6">
    <h3 class="font-headline-sm text-headline-sm text-primary">Rapor Kelas</h3>
    <form method="get" class="flex items-center gap-2">
        <select name="tahun_ajar_id" class="px-3 py-2 bg-surface-container-low border-2 border-outline/60 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary/20" onchange="this.form.submit()">
            ...
            <option value="<?= $ta['id'] ?>" <?= ($selectedTa == $ta['id'] || (!$selectedTa && $ta['is_current'])) ? 'selected' : '' ?>>
                <?= esc($ta['tahun']) ?>
            </option>
            <?php endforeach; ?>
        </select>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <a href="<?= base_url('walas/rapor/kelas/' . $r['id']) ?>" class="rapor-card glass-card p-6 rounded-2xl hover:bg-white transition-colors group"
       data-ta="<?= $r['tahun_ajar_id'] ?? '' ?>">
        ...
        <span class="text-primary text-sm font-bold flex items-center gap-1">
            Lihat Rapor <span class="material-symbols-outlined text-sm">arrow_forward</span>
        </span>
    </a>
    <?php endforeach; ?>
    <?php else: ?>
    <div class="col-span-full text-center py-12 text-on-surface-variant text-sm">Tidak ada rombel untuk tahun ajar ini</div>
    <?php endif; ?>
</div>
<?= $this->endSection() ?>
