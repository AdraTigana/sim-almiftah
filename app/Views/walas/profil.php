...
<?= view('components/_breadcrumb', ['items' => [['label' => 'Dashboard', 'url' => base_url('walas')], ['label' => 'Profil']]]) ?>
...
<div class="max-w-2xl mx-auto">
    <div class="glass-card rounded-3xl overflow-hidden shadow-sm shadow-primary/5">
        ...
        <form method="post" action="<?= base_url('walas/profil/update') ?>" data-offline="true" class="p-8 space-y-6">
            <?= csrf_field() ?>
            ...
            <div>
                <label class="text-xs text-outline uppercase tracking-wider font-semibold block mb-1">Status</label>
                <span class="px-3 py-1 <?= ($user['is_active'] ?? 0) ? 'bg-primary-container text-primary' : 'bg-error-container text-error' ?> rounded-full text-xs font-bold"><?= ($user['is_active'] ?? 0) ? 'Aktif' : 'Nonaktif' ?></span>
            </div>
